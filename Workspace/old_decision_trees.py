...
def DT_train_binary(X, Y, max_depth):
    ...
    if entropyN[0] == 0 and entropyY[0] == 0:
        ...
        print("entropy y is 0")
        # instruction.append
        rule2 = [max_index], 0 #use this result in the next one
    print("Rule2: {}".format(rule1))
    total_entropy, set_length = find_split_entropy_rule1(max_index, 0, X, Y, rule, rule1)
    print(total_entropy)
    print(set_length)

    num1 = find_split_entropy_rule2(0, 1, X, Y, rule, rule1, rule2)
    num2 = find_split_entropy_rule2(0, 0, X, Y, rule, rule1, rule2)
    branch_entropies.append(find_IG(num1, num2, total_entropy, set_length))
    ...
    entropyN = find_split_entropy_rule2(max_index, 0, X, Y, rule, rule1, rule2)
    entropyY = find_split_entropy_rule2(max_index, 1, X, Y, rule, rule1, rule2)
...
def find_split_entropy_rule2(column, value, X, Y, rule, rule1, rule2):
    split_positive = 0
    split_total = 0
    for idx in enumerate(X):
        if idx[1][column] == value and idx[1][rule[0]] == rule[1] and idx[1][rule1[0]] == rule1[1] and idx[1][rule2[0]]\
                == rule1[1]:
